...
Assise sur le package geosphere, la fonction MinDist est évaluée à partir de l'une des trois distances prédéfinies dans le package (distHaversine, distVincentyEllipsoid, distVincentySphere). Elle mesure la plus proche distance entre deux coordonnées géographiques et identifie le point correspondant à cette plus courte distance.
...
A noter que si la distance précalculée VincentyEllipsoid est plus précise, son coût est polynomial, veillez donc à avoir la ressource nécessaire pour l' utiliser.
